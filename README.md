...
手機優先的 3D 物理融合遊戲。V5.4 採「美術優先」混合架構：顯示單一、無接縫的高品質預渲染杯子與跑道，同時由 V5 的 Rapier 3D 世界負責跑道、護欄、前牆及杯身碰撞。
...
正式版以 Git tag 與 Sites 的不可變版本共同保存。`v5.0.0` 至 `v5.4.0` 都可獨立復原；回復線上版本時部署既有 Sites 版本即可，不必重新產生資產。
