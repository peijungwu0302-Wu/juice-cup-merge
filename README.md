...
手機優先的 3D 物理融合遊戲。V5.5 採「美術優先」混合架構：顯示單一、無接縫的高品質預渲染杯子與跑道，同時由 V5 的 Rapier 3D 世界負責跑道、護欄、前牆及杯身碰撞。

## V5.5 重點

- 果汁珍藏、聖代工房、銀河酒窖各有七級獨立裝飾與配色，不共用同一批水果。
- 由原始高解析美術圖精確裁出 21 張獨立 RGBA 杯子，保留透明安全邊界，不會再切到相鄰杯子。
- 精緻杯改用保持垂直、僅水平朝向鏡頭的 2.5D 平面；每杯使用自己的杯腳留白資料，杯腳不再因全向 Sprite 投影而懸浮。
- 杯腳使用短而深的接觸陰影，加上低透明度的環境陰影；兩層陰影貼合斜坡表面，不再用偏離杯底的大型固定橢圓。
- 固定美術攝影機再次微調近端縱深，杯身主接觸帶、左右護欄與前牆都和 Rapier 碰撞世界使用同一份規格。
...
- 角度與力度皆關閉時，預設可左右定位後直接放手直射；按壓至少 0.05 秒並可向下拖曳取消，設定內可關閉並回到前滑門檻操作。
...
正式版以 Git tag 與 Sites 的不可變版本共同保存。`v5.0.0` 至 `v5.5.0` 都可獨立復原；回復線上版本時部署既有 Sites 版本即可，不必重新產生資產。
